...
import { domain } from '../utils/denbug.js';
...
class AIService extends EventEmitter {
  constructor() {
    ...
    this.processMessage = async (message, socket) => {
      ...
      this.logger.log('Processing user intent:', { text, context });
  ...
  setupServer(httpServer) {
    ...
    this.logger.log('WebSocket server initialized with CORS configuration');
    ...
    io.on('connect', (socket) => {
      ...
      this.logger.log('Sending welcome message to client:', socket.id);
      socket.emit('chat:welcome', {
        message: 'Welcome to Serra!'
      });
      this.logger.log('Welcome message sent to client:', socket.id);
      ...
      socket.on('engine:state', (state) => {
        this.logger.log('Received engine state update from client:', socket.id);
        const client = this.clients.get(socket.id);
        if (client) {
          client.engineState = state;
          this.logger.log('Updated engine state for client:', socket.id);
        }
      });
